            ...
            await step.run('check-if-task-is-completed', async () => {
                ...
                if(task.status !== "DONE"){
                    await step.run('send-task-reminder-email', async () => {
                        await sendEmail({
                            to: task.assignee.email,
                            subject: `Reminder: Task " ${task.project.name}" is Due`,
                            body: `<div style ="max-width: 600px;"
            <h2>Hi ${task.assignee.name}, 👋 </h2>
...
export const functions = [syncUserCreation,syncUserDeletion,syncUserUpdation,syncWorkspaceCreation,syncWorkspaceUpdation,syncWorkspaceDeletion,syncWorkspaceMemberCreation,sendTaskAssigneeEmail];